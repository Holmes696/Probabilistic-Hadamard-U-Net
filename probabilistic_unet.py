...
class Fcomb(nn.Module):
    ...
    def __init__(self, num_filters, latent_dim, num_output_channels, num_classes, no_convs_fcomb, initializers, use_tile=True):
        ...
        if self.use_tile:
            ...
            #we can change the input dimension here
            layers.append(nn.Conv2d(3, self.num_filters[0], kernel_size=1))
            layers.append(nn.ReLU(inplace=True))
...
class ProbabilisticUnet(nn.Module):
    ...
    def forward(self, patch, segm, training=True):
        ...
        self.unet_features,self.loss_tv= self.unet.forward(patch,False)
    ...
    def elbo(self, segm, analytic_kl=True, reconstruct_posterior_mean=False):
        ...
        return -(self.reconstruction_loss + self.beta * self.kl+self.loss_tv)
